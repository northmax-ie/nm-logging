...
## Foreign logging remains separate

nm-logging does not ingest, adapt, suppress, reconfigure, redirect, or otherwise
manage standard-library, framework, or third-party logging. Those components
retain the logging configuration supplied by the consuming application.

nm-logging provides no standard-library `logging.Handler`, sink adapter, or
other bridge that routes foreign records into authoritative NorthMax operational
or audit storage. NorthMax records emitted through nm-logging are likewise not
forwarded into standard-library or root logging. There is no bridge in either
direction.
