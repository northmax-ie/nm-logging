...
NorthMax's logging contract applies only to authoritative application-owned
logging emitted through nm-logging.

nm-logging does not ingest, adapt, suppress, reconfigure, redirect, or otherwise
manage Python standard-library, framework, runtime, or third-party logging.
Foreign logging remains entirely under the consuming application's and foreign
component's logging configuration. nm-logging must not modify the root logger or
any foreign logger, handler, level, or propagation setting.

No nm-logging-provided standard-library ``logging.Handler``, sink adapter, or
other bridge may route foreign records into authoritative NorthMax operational
or audit storage. Conversely, NorthMax records emitted through nm-logging are not
forwarded into standard-library or root logging.

There is intentionally no bridge in either direction.
