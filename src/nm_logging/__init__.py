...
The concrete ``JsonlReader`` lives deliberately just outside this top-level
surface at ``nm_logging.reader.jsonl``, because nothing in the write-side package
may import the reader (§22). Standard-library, framework, and third-party logging
is outside nm-logging's scope: there is no bridge in either direction (§25).
"""
